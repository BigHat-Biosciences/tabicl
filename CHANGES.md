...
- Support backends without memory-introspection APIs, enabling `device="xla"` (`torch_xla` on TPU and AWS Neuron/Inferentia). Auto-batching is now gated on whether the backend actually exposes a free-memory query (`mem_get_info`, or the MPS `recommended_max_memory`/`current_allocated_memory` pair) rather than on the device being CPU. Previously such a backend fell through to the accelerator path, where `get_available_gpu_memory()` returning `0.0` — meaning "unknown", not "nothing free" — collapsed batch sizing and forced an output offload that failed with a misleading `CPU memory allocation failed` error. `backend_is_available()` also resolves backends that ship as their own top-level module rather than as `torch.<device_type>`; `xla` is deliberately excluded from the default device preference because importing `torch_xla` initializes the PJRT runtime as a side effect, so it must be requested explicitly.
